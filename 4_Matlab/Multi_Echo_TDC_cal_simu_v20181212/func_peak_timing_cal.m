...
xIntercept = param1 / 10e-12; % unit change from 1s to 10ps
%%
switch calTypeID
    case {1, 'width8'}
        typeID = 1; 
    ...
    case {14, 'fixed-rising-cal'}
        typeID = 14;
        time_shift = delta0 - 2565 ; 
        fprintf('Current Calibration Method : %s .\n', calTypeID); 
    case {15, 'fixed-falling-cal'}
        typeID = 15;
        time_shift = delta0 - 2892 ;
        fprintf('Current Calibration Method : %s .\n', calTypeID);        
end % end switch
...
output_data = time_shift * 10e-12; % unit 10ps -> 1s

fprintf('cal value : %g ns at input parameter %g ns.\n', [output_data*1e9, param1*1e9]);  
